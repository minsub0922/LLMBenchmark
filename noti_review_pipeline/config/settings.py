...
DEFAULT_FEEDBACK_PROFILE = os.environ.get('FEEDBACK_PROFILE', 'none')
DEFAULT_SAVE_HISTORY = os.environ.get('SAVE_HISTORY', '1') == '1'
...
class ReviewConfig:
    ...
    feedback_profile: str = DEFAULT_FEEDBACK_PROFILE
    ...
    def run_slug(self) -> str:
        parts = [self.checks_slug(), f'p{self.max_passes}', self.mode]
        if self.feedback_profile and self.feedback_profile != 'none':
            parts.append(self.feedback_profile)
        if self.dry_run:
            parts.append('dry')
